...
public class PacketKeeper {
	...
	/**
	 * Persist anonymous profile JSON for a registration ID in the configured object store.
	 *
	 * @param id the registration ID
	 * @param profileJson the anonymous profile as a JSON string
	 * @return {@code true} if the object was stored successfully
	 */
	public boolean putAnonymousProfile(String id, String profileJson) {
		try (ByteArrayInputStream stream = new ByteArrayInputStream(profileJson.getBytes(java.nio.charset.StandardCharsets.UTF_8))) {
			...
			LOGGER.error(PacketManagerLogger.SESSIONID, PacketManagerLogger.REGISTRATIONID, id,
					"putAnonymousProfile failed: " + ExceptionUtils.getStackTrace(e));
			throw new ObjectStoreAdapterException(
					"Failed to persist anonymous profile in object store: " + e.getMessage(), e);
		}
	}

	/**
	 * Fetch the anonymous profile JSON previously stored for a registration ID.
	 *
	 * @param id the registration ID
	...
	public String getAnonymousProfile(String id) {
		try (InputStream is = getAdapter().getObject(PACKET_MANAGER_ACCOUNT, id,
			...
			if (e.getMessage() != null
					&& (e.getMessage().contains(OBJECT_DOESNOT_EXISTS) || e.getMessage().contains(STATUS_404))) {
				return null;
			}
			LOGGER.error(PacketManagerLogger.SESSIONID, PacketManagerLogger.REGISTRATIONID, id,
					"getAnonymousProfile failed: " + ExceptionUtils.getStackTrace(e));
			throw new ObjectStoreAdapterException(
					"Failed to read anonymous profile from object store: " + e.getMessage(), e);
		}
	}
}
